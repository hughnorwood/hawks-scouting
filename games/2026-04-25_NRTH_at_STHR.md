...
- **Date and time:** Sat Apr 25, 12:00 PM - 1:00 PM ET
...
| 11 | 6 | Bottom | C England | Double Play | grounds into a double play, shortstop H Lagasse to first baseman S Tarantino, M Guard out advancing to 2nd, C England doubled off at 1s | 3 | | |
...
- Play log runs by inning: NRTH (1 run in 7th) = 1 total; STHR (0 runs identified) = 0 total
- Line score inning totals: NRTH 0+1+1+0+0+1+1 = 4; STHR 0+0+0+0+5+0+0 = 5
- Reported team R totals: NRTH = 4, STHR = 5
- **Conflict:** Play log shows only 1 run scored (play #15), but line score and team totals show NRTH = 4, STHR = 5. The discrepancy is unresolved.

**Mandatory Verification — Hit Totals:**
- Hits from play log: Play #1 (M Bolak single), Play #9 (M Guard single), Play #14 (V Wong single), Play #15 (G decker home run) = 4 total hits
- Reported team H totals: NRTH = 6, STHR = 7
- **Conflict:** Play log shows 4 hits total, but reported totals show NRTH = 6, STHR = 7.

**Mandatory Verification — Errors:**
- No error events identified in the play log
- Reported team E totals: NRTH = 4, STHR = 2
- **Conflict:** Play log shows 0 errors, but reported totals show NRTH = 4, STHR = 2.

**Flag #1:** Line score vs. log contradiction - The line score shows runs in multiple innings for both teams, but the play log only contains entries from the 5th inning onward and only records 1 run scored. Earlier innings appear to be missing from the log.

**Flag #2:** Undocumented pitching transition - Play #1: G decker identified as pitcher in play #3 description without a preceding "Lineup changed" entry.

**Flag #3:** Undocumented pitching transition - Play #16: V Cagle identified as pitcher in play descriptions without a preceding "Lineup changed" entry.

**Flag #4:** Unusual play notations - Play #2: Description mentions "M Kim out advancing to 1st" in what appears to be a sacrifice bunt play, which is internally contradictory as Kim is not the listed batter.